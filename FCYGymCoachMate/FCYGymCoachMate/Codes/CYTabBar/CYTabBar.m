...
- (void)setItems:(NSArray<UITabBarItem *> *)items{
    ...
    for (int i=0; i<items.count; i++)
    {
        ...
        else
        {
            ...
            button.design = SetBtnContentLeftAndRight;
            btn = button;
            //Add Observer
...
- (void)layoutSubviews{
    ...
    int mid = count/2;
    CGRect rect = CGRectMake(10, 0, self.bounds.size.width/count,self.bounds.size.height);
    ...
    for (int i=0; i<count; i++)
    {
        if (i == mid && self.centerBtn!= nil)
        {
            ...
            : CGRectMake(rect.origin.x,
                         0,
                         rect.size.width-20,
                         rect.size.height);   //  修改了 rect
        }
        else if (i == 0)
        {
            self.btnArr[0].frame = rect;
        }else {
            self.btnArr[1].frame = CGRectMake(rect.origin.x-20, 0, rect.size.width, rect.size.height);
        }
