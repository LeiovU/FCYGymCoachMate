...
+(NSArray<CYContact *> *)searchText:(NSString *)searchText inDataArray:(NSArray<CYContact *> *)dataArray {
    NSMutableArray<CYContact *> *results = [NSMutableArray array];
    // 参数不合法
    ...
    if (isIncludeChineseInNSString(searchText)) {  //输入了中文-只查询中文
        for (CYContact *contact in dataArray) {
        ...
        for (CYContact *contact in dataArray) {
            if (isIncludeChineseInNSString(contact.name)) {// 待查询中有中文--转为拼音
                ...
                if ([contact.name hasPrefix:@"曾"] && [completeHasSeparatorPinyin hasPrefix:@"c"]) {
                    completeHasSeparatorPinyin = [completeHasSeparatorPinyin stringByReplacingCharactersInRange:NSMakeRange(0, 0) withString:@"z"];
                }
                // 删除分隔符
